...
void *ThreadBroad1(void *threadArgs)
{
	...
	unsigned short broadcastPort = ((struct ThreadArgs *) threadArgs) -> broadcastPort;
	...
	sendto(sock, echoBuffer, strlen(echoBuffer), 0,(struct sockaddr *)&broadcastAddr, sizeof(broadcastAddr));
	}
...
void *ThreadMain2(void *threadArgs)
{
	...
	{
	pthread_mutex_lock(&shared.mutex);
	read_message(msgqid, (struct mymsgbuf *)&qbuf,qtype, recvBuff);
...
int main(int argc, char *argv[])
{
	...
	pthread_t threadID;
	unsigned short broadcastPort = 1500;
	...
	bind(threadArgs -> listenfd, (struct sockaddr *)&serv_addr, sizeof (serv_addr));

	listen(threadArgs -> listenfd, 10);
		
	while (1) 
	{
		threadArgs -> connfd = accept(threadArgs -> listenfd, (struct sockaddr *)NULL, NULL); 
		...
		if (pthread_create(&threadID, NULL, ThreadBroad1, (void *) threadArgs) != 0) {
			perror("Creating thread");
			return EXIT_FAILURE;
		}
		
		if (pthread_create(&threadID, NULL, ThreadBroad2, (void *) threadArgs) != 0) {
			perror("Creating thread");
			return EXIT_FAILURE;
		}
		...
		{
		if (pthread_create(&threadID, NULL, ThreadMain2, (void *) threadArgs) != 0) {
			perror("Creating thread");
			return EXIT_FAILURE;
		}
		}	
